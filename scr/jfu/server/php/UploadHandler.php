...
class UploadHandler
{
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            @mysql_query("INSERT INTO pictures (pkey, pic, usr, since, status) VALUES ('".hash('sha512',date("YmdHis").$_SESSION["ukey"])."', '$file_path', '".$_SESSION["ukey"]."', '".date("Y-m-d H:i:s")."', '1')");
    ...
    public function post($print_response = true) {
        ...
        if ($upload && is_array($upload['tmp_name'])) {
            ...
            foreach ($upload['tmp_name'] as $index => $value) {
                ...
                #ins("pictures","pkey, pic, usr, since, status","'".hash('sha512',date("YmdHis").$_SESSION["ukey"])."', '$file_name', '".$_SESSION["ukey"]."', '".date("Y-m-d H:i:s")."', '1'");
            }
        } else {
            ...
            #@mysql_query("INSERT INTO pictures (pkey, pic, usr, since, status) VALUES ('".hash('sha512',date("YmdHis").$_SESSION["ukey"])."', '$file_name', '".$_SESSION["ukey"]."', '".date("Y-m-d H:i:s")."', '1')");
        }
        ...
        if ($success) {
            foreach($this->options['image_versions'] as $version => $options) {
                if (!empty($version)) {
                    ...
                    if (is_file($file)) {
                        unlink($file);
                        mysql_query("DELETE FROM pictures WHERE pic = '$file'") or die(mysql_error());
                    }
                }
            }
        }
